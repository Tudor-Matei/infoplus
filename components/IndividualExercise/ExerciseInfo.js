  ...
  return (
    <>
      <div className="exercise-info">
        <HeaderTitleExercise needsHint={needsHint} userSolutions={userSolutions} officialSolution={officialSolution} />
...
function UserSolution({ name, surname, username, datePublished, testsPoints, solution }) {
  return (
    <>
      <div className="user-solution" style={{ backgroundColor: `var(--${decideColourFromPoints(testsPoints)})` }}>
        <div className="user-solution__datum">
          <IconBubble className="user-solution__icon" icon="user" /> {name} {surname} ({username})
        </div>
        <div className="user-solution__datum">
...
function HiddenExerciseDataButton({ whatToGet, children }) {
  ...
  const { exerciseId, hiddenExerciseData, setHiddenExerciseData, userSolutions } = useContext(ExerciseInfoData);
  ...
  const fetchHiddenExerciseData = useCallback(() => {
    if (whatToGet === "userSolutions")
      return setPanelData({
        value: userSolutions.map((userSolution, i) => <UserSolution key={`userSolution_${i}`} {...userSolution} />),
      });

    const isOfficialSolution = whatToGet === "officialSolution";
    if (!hiddenExerciseData) {
      ...
      return fetch("/api/exercises/hiddenData", {
        ...
        .then(({ data, err }) => {
          ...
          setPanelData({
            value: isOfficialSolution ? <code>{atob(data[whatToGet])}</code> : data[whatToGet],
          });
          setHiddenExerciseData({ value: { ...data, officialSolution: atob(data.officialSolution) } });
          setDisabled(false);
        });
    }

    return setPanelData({
      value: isOfficialSolution ? <code>{hiddenExerciseData[whatToGet]}</code> : hiddenExerciseData[whatToGet],
    });
  }, [hiddenExerciseData, whatToGet, userSolutions]);

  return (
    <>
      <button className="button--outline" disabled={isDisabled} onClick={fetchHiddenExerciseData}>
        {children}
      </button>
      <style jsx>{`
        button {
...
function HeaderTitleExercise({ needsHint, userSolutions, officialSolution }) {
  return (
    <HeaderSection title="Enunțul exercițiului">
      {needsHint && <HiddenExerciseDataButton whatToGet="hint">Indiciu</HiddenExerciseDataButton>}
      {userSolutions ? (
        <HiddenExerciseDataButton whatToGet="userSolutions">Soluțiile tale: {userSolutions}</HiddenExerciseDataButton>
      ) : null}
      {officialSolution && (
        <HiddenExerciseDataButton whatToGet="officialSolution">Soluția oficială</HiddenExerciseDataButton>
      )}
    </HeaderSection>
  );
}
