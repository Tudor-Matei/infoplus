...
export default function Revision() {
  ...
  const formattedGeneralData = useCallback(() => {
    const uglyTitleAlias = generalData.category[0];
    const uglyChapter = chapters[generalData.grade].find(({ titleAlias }) => titleAlias === uglyTitleAlias);
  ...
  const submitExerciseData = useCallback(() => {
    ...
    data
      .then((r) => r.json())
      .then(({ ok = false, err = "A apărut o eroare internă, vă rugăm să ne scuzați." }) => {
        if (!ok) {
  ...
  return (
    <>
      ...
      <div className="exercise-data-panel">
        ...
        <Accordion className="accordion--revision" borderAccent="secondary" title="Conținutul exercițiului">
          <DataCategory type="contentData" data={contentData} />
        </Accordion>
        <Accordion className="accordion--revision" borderAccent="tertiary" title="Date de intrare/ieșire">
