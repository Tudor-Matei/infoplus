...
export async function getServerSideProps({ req, res }) {
  ...
  const { data: dataDb, err: errDb } = await performDatabaseOperation(async (db) => {
    ...
    if (!foundUser)
      return exitOperationWith("S-a intâmplat ceva ciudat, aveți datele de autentificare prezente, dar invalide.");
...
export default function Dashboard({ authenticated, userData, latestExercisesData, solvedFailedExercises, err }) {
  return (
...
function InnerCircleSVG({ className }) {
  return (
    ...
    >
      <circle cx="156" cy="156" r="154" stroke="var(--accent-primary)" strokeWidth="4" strokeDasharray="24 24" />
    </svg>
  );
}
